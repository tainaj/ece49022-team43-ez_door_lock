...
uint8_t character_buffer2[R502_TEMPLATE_SIZE];
...
{
    // this is where you would store or otherwise do something with the image
    uint8_t *ptr = &character_buffer[up_char_size];
    ...
    while(total < data_len){
        ...
        //}
        //printf("\n");
        ptr[total] = data[total];
        total++;
    }
}

void print_buffer(uint8_t *ptr) {
    printf("Template hex: \n");
    uint8_t *data = &ptr[0];
    int box_width = 16;
...
TEST_CASE("UpChar_RegModel_DownChar_Store", "[fingerprint processing][userInput]")
{
    // Steps:
    // 1: Test "RegModel" - create template of the correct fingerprint of your choice
    // 2: Test this test - insert false fingerprint. Test should reject it, and store the correct one instead.
    ...
    // move to second buffer
    for (int i = 0; i < R502_TEMPLATE_SIZE; i++) {
        character_buffer2[i] = character_buffer[i];
        character_buffer[i] = 0;
    }

    // Reg-model (distorts original buffers)
    // 1st fingerprint: generate image from fingerprint
    wait_with_message("Place finger on sensor, then press enter\n");
    ...
    // Attempt down_char
    buffer_id = 1;
    err = R502_down_char(&R502, starting_data_len, buffer_id, character_buffer2, &conf_code);
    TEST_ESP_OK(err);
    TEST_ASSERT_EQUAL(R502_ok, conf_code);
    ...
    // Compare the buffers
    for (int i = 0; i < R502_TEMPLATE_SIZE; i++) {
        TEST_ASSERT_EQUAL(character_buffer[i], character_buffer2[i]);
    }
...
TEST_CASE("DownChar_UpChar", "[fingerprint processing][dataExchange]")
{
    populate_buffer();
